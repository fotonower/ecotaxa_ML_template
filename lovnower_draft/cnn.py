...
def Create(
    ...
    if extra_input_shape > 0:
        vector_input = tf.keras.Input(shape=(extra_input_shape,))
    ...
    # Add and concatenate extra input if a feature vector is given
    if extra_input_shape > 0:
        out = layers.Concatenate()([out, vector_input])
    ...
    # Define model
    if extra_input_shape > 0:
        model = tf.keras.Model(inputs=[image_input, vector_input], outputs=out)
    else:
