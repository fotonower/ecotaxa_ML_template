...
parser.add_argument('--use_features', type=int, default=0, help='Add features as extra input to the network')
...
nb_of_features = -1
features_lists_train, features_lists_val, features_lists_test = None, None, None

# define special data generators if we use features
if args.use_features:
    features_lists_train = df_train.drop(['img_path', 'label'],axis=1).values
    features_lists_val = df_val.drop(['img_path', 'label'],axis=1).values
    features_lists_test = df_test.drop(['img_path', 'label'],axis=1).values
    nb_of_features = len(df_train.columns) - 2
    
# define data generators
train_batches = dataset.EcoTaxaGenerator(
    images_paths=df_train['img_path'].values,
    input_shape=input_shape,
    features_lists=features_lists_train,
...
val_batches = dataset.EcoTaxaGenerator(
    ...
    input_shape=input_shape,
    features_lists=features_lists_val,
...
test_batches = dataset.EcoTaxaGenerator(
    ...
    input_shape=input_shape,
    features_lists=features_lists_test,
...
if (my_cnn is not None) :
    ...
    my_cnn = cnn.Create(
        ...
        # optional extra input shape
        extra_input_shape=nb_of_features
    )
